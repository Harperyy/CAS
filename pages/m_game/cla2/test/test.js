...
Page({
  ...
  onLoad: function (options) {
   ...
   // wx.setNavigationBarTitle({ title: options.testId }) // 动态设置导航条标题
    ...
    this.setData({
      questionList: app.globalData.questionList['301-302'],  // 拿到答题数据
      testId: '301-302' // 课程ID
    })
    ...
    this.setData({
      shuffleIndex: this.shuffle(count).slice(0, 8) // 生成随机题序 [2,0,3] 并截取num道题
    })
  },
  /*
  * 数组乱序/洗牌
  ...
  */
  nextSubmit: function(){
    ...
    if (this.data.index < this.data.shuffleIndex.length - 1) {
      ...
      app.globalData.c2_num3= this.data.totalScore
      wx.redirectTo({
        url: '../../yd2/sp3/sp',
      })
